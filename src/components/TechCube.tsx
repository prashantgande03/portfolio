...
export function TechCube({ size = 120 }: { size?: number }) {
  const half = size / 2;
  const transforms = [
    ...
    <div className="perspective" style={{ width: size, height: size }}>
      <div className="relative h-full w-full animate-spin-y">
        {faces.map((f, i) => (
          <div
            key={f.label}
            className="absolute inset-0 flex items-center justify-center rounded-lg border-2 font-bold uppercase tracking-widest backdrop-blur-sm"
            style={{
